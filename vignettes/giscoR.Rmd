...
You can select specific countries by name (in any language), ISO 3 codes or Eurostat codes. The only restriction is that you can't miss country names, ISO3 and Eurostat codes on one single call.


```{r africa_north, fig.width=7, fig.asp=1}

africa_north <- gisco_get_countries(resolution = "10", country = c("Argelia","Morocco", "Egipto","Tunisia","Libia"))
...
```{r belgium, fig.width=7, fig.asp=1}

bel_60 <- gisco_get_countries(resolution = "60", country = "Belgique")
bel_20 <- gisco_get_countries(resolution = "20", country = "Belgique")
bel_10 <- gisco_get_countries(resolution = "10", country = "Belgique")
bel_03 <- gisco_get_countries(resolution = "03", country = "Belgique")

opar <- par(no.readonly = TRUE)
par(mfrow = c(2, 2))
plot(st_geometry(bel_60), main = "60M")
plot(st_geometry(bel_20), main = "20M")
plot(st_geometry(bel_10), main = "10M")
plot(st_geometry(bel_03), main = "03M")
title(sub = gisco_attributions(copyright = "FALSE"), cex.sub = 0.7)
par(opar)
...
```{r giscoR_eurostat, fig.height=8, fig.width=8, message=FALSE, warning=FALSE}
library(eurostat)
library(cartography)
library(colorspace)

nuts2 <- gisco_get_nuts(year = "2016",
                        nuts_level = "2",
                        epsg = "3035")
